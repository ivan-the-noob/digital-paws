...
try {
    ...
    while ($row = $result->fetch_assoc()) {
      $cost_str = $row['cost']; // Keep the cost as a string (e.g., '450-1000')
      $discount = (float) $row['discount']; // Discount is still numeric
  
      // Don't perform any calculations on the cost, just store it as-is
      $discounted_cost = $cost_str; // Simply keep the range as the discounted cost

      $service_data = [
          'service_name' => $row['service_name'],
          'cost' => htmlspecialchars($cost_str), // Display cost as the string (450-1000)
          'discount' => $row['discount'],
          'discounted_cost' => $discounted_cost, // Keep discounted cost as a string
